  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-gray-800">预算</h2>
      ...
      {showAdd && (
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-4 border border-indigo-200 shadow-sm space-y-3">
          <p className="text-sm font-medium text-gray-600">新建预算</p>
          <div className="space-y-2.5">
            <select value={addCat} onChange={(e) => setAddCat(e.target.value)}
              className="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-xl bg-white outline-none focus:border-indigo-300">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="flex items-center border border-gray-200 rounded-xl px-3 bg-white focus-within:border-indigo-300">
                ...
                <input type="number" min="0" step="100" value={addAmount} onChange={(e) => setAddAmount(e.target.value)}
                  placeholder="金额" className="w-full py-2.5 text-sm outline-none" autoFocus inputMode="numeric" />
              </div>
              <select value={addType} onChange={(e) => setAddType(e.target.value as 'monthly' | 'annual')}
                className="px-3 py-2.5 text-sm border border-gray-200 rounded-xl bg-white outline-none focus:border-indigo-300">
            ...
            <input type="text" value={addNote} onChange={(e) => setAddNote(e.target.value)}
              placeholder="备注（可选）" className="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-xl bg-white outline-none focus:border-indigo-300" />
          </div>
          <div className="flex gap-2">
            <button onClick={handleAdd}
              className="px-5 py-2.5 bg-indigo-500 text-white text-sm rounded-xl hover:bg-indigo-600 transition-colors">确认</button>
            <button onClick={() => setShowAdd(false)}
              className="px-5 py-2.5 bg-gray-100 text-gray-600 text-sm rounded-xl hover:bg-gray-200 transition-colors">取消</button>
          </div>
        </div>
      )}
              ...
              <div key={b.categoryId} className={`bg-white/80 backdrop-blur-sm rounded-2xl p-4 border shadow-sm transition-all ${isEditing ? 'border-indigo-300 ring-1 ring-indigo-200' : 'border-gray-100'}`}>
                  ...
                  <div className="flex items-center gap-1">
                    {!isEditing && (
                      <span className="text-[12px] text-gray-500 mr-1">
                        {b.budgetType === 'monthly' ? <>月¥{b.amount.toLocaleString()}</> : <>年¥{b.amount.toLocaleString()}</>}
                      </span>
                    )}
                    <button onClick={() => startEdit(b.categoryId)}
                      className="text-[12px] text-indigo-500 hover:text-indigo-700 px-1.5 py-1">修改</button>
                    <button onClick={() => dispatch({ type: 'DELETE_BUDGET', payload: b.categoryId })}
                      className="text-[12px] text-gray-300 hover:text-red-400 px-1.5 py-1">删除</button>
                  </div>
                </div>

                {/* Edit form */}
                {isEditing && (
                  <div className="space-y-2 mb-3 bg-indigo-50/50 rounded-xl p-3">
                    <div className="grid grid-cols-2 gap-2">
                      <div className="flex items-center border border-gray-200 rounded-lg px-2.5 bg-white">
                        ...
                        <input type="number" min="0" step="100" value={editAmount} onChange={(e) => setEditAmount(e.target.value)}
                          className="w-full py-2 text-sm outline-none" autoFocus inputMode="numeric" />
                      </div>
                      <select value={editType} onChange={(e) => setEditType(e.target.value as 'monthly' | 'annual')}
                        className="px-2.5 py-2 text-sm border border-gray-200 rounded-lg bg-white outline-none">
                    ...
                    <div className="flex gap-2">
                      <input type="text" value={editNote} onChange={(e) => setEditNote(e.target.value)}
                        placeholder="备注" className="flex-1 px-2.5 py-2 text-sm border border-gray-200 rounded-lg bg-white outline-none" />
                      <button onClick={saveEdit}
                        className="px-4 py-2 bg-indigo-500 text-white text-sm rounded-lg hover:bg-indigo-600 transition-colors">保存</button>
                      <button onClick={() => setEditingId(null)}
                        className="px-4 py-2 bg-gray-100 text-gray-600 text-sm rounded-lg hover:bg-gray-200 transition-colors">取消</button>
                    </div>
                  </div>
                )}
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2 text-[12px]">
                    <span className="text-gray-400 w-8 shrink-0">年度</span>
                    ...
                    <span className="text-gray-500 text-right shrink-0">¥{ySpent.toLocaleString()} / ¥{annualTotal.toLocaleString()}</span>
                    <span className={`w-10 text-right font-semibold shrink-0 ${yPct > 100 ? 'text-rose-500' : 'text-gray-500'}`}>{yPct.toFixed(1)}%</span>
                  </div>
                  {b.budgetType === 'monthly' && (
                    <div className="flex items-center gap-2 text-[12px]">
                      <span className="text-gray-400 w-8 shrink-0">本月</span>
                      <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full transition-all ${
                      ...
                      <span className="text-gray-500 text-right shrink-0">¥{mSpent.toLocaleString()} / ¥{b.amount.toLocaleString()}</span>
                      <span className={`w-10 text-right font-semibold shrink-0 ${mPct > 100 ? 'text-rose-500' : 'text-gray-500'}`}>{mPct.toFixed(1)}%</span>
                    </div>
                  )}
                </div>
              </div>
