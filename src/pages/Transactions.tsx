  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-gray-800">流水记录</h2>
      ...
      {/* Filters */}
      <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-3 border border-gray-100 shadow-sm space-y-2">
        <div className="flex flex-wrap gap-2">
          <select value={filter} onChange={(e) => setFilter(e.target.value as 'all' | 'income' | 'expense')}
            className="px-3 py-1.5 text-xs border border-gray-200 rounded-lg bg-white outline-none focus:border-indigo-300">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="🔍 搜索备注..."
            className="px-3 py-1.5 text-xs border border-gray-200 rounded-lg bg-white outline-none focus:border-indigo-300 flex-1 min-w-[100px]" />
        </div>
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-gray-400">日期范围：</span>
          <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)}
            className="px-2 py-1 text-xs border border-gray-200 rounded-lg bg-white outline-none focus:border-indigo-300" />
          <span className="text-xs text-gray-400">至</span>
          <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)}
            className="px-2 py-1 text-xs border border-gray-200 rounded-lg bg-white outline-none focus:border-indigo-300" />
          {(dateFrom || dateTo) && (
            <button onClick={() => { setDateFrom(''); setDateTo(''); }}
              className="text-xs text-indigo-500 hover:text-indigo-700">清除</button>
          )}
        </div>
      </div>
