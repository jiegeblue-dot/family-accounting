      ...
      <div className="bg-white rounded-t-3xl md:rounded-2xl p-5 md:p-6 w-full max-w-md shadow-2xl max-h-[85vh] overflow-auto safe-bottom" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center border-2 border-gray-100 rounded-xl px-4 py-3 focus-within:border-indigo-300 transition-colors bg-gray-50">
              <span className="text-gray-400 mr-1 text-lg">¥</span>
              <input type="number" step="0.01" min="0.01" required value={amount}
                onChange={(e) => setAmount(e.target.value)} placeholder="0.00"
                className="flex-1 outline-none text-xl font-bold bg-transparent w-full" autoFocus
                inputMode="decimal" />
            </div>
          </div>
            ...
            <div className="grid grid-cols-5 gap-1.5">
              {filteredCategories.map((cat) => (
                <button key={cat.id} type="button" onClick={() => setCategoryId(cat.id)}
                  className={`flex flex-col items-center gap-0.5 p-2 md:p-2 rounded-xl text-[10px] md:text-[11px] transition-all ${
                    categoryId === cat.id
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="date" required value={date} onChange={(e) => setDate(e.target.value)}
                className="w-full border-2 border-gray-100 rounded-xl px-3 py-3 text-sm outline-none bg-gray-50 focus:border-indigo-300 transition-colors" />
            </div>
            <div>
          ...
          <div>
            ...
            <input type="text" value={note} onChange={(e) => setNote(e.target.value)}
              placeholder="添加备注..." className="w-full border-2 border-gray-100 rounded-xl px-4 py-3 text-sm outline-none bg-gray-50 focus:border-indigo-300 transition-colors" />
          </div>

          {/* Actions */}
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose}
              className="flex-1 py-3.5 text-[15px] font-medium rounded-xl bg-gray-100 text-gray-600 hover:bg-gray-200 transition-colors">
              取消
            </button>
            <button type="submit"
              className={`flex-1 py-3.5 text-[15px] font-bold rounded-xl text-white shadow-lg transition-all ${
